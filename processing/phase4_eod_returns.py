...
df = eik.GetFinancialBySymbols(
        symbols = all_symbols, 
        columns=['OPENPRICE','CLOSEPRICE','MARKETCAP'], 
        date_from=date_from, date_to=date_to)

df = pd.concat([df,fsi_index, telco_index], axis=1)
...
for sym in all_symbols + ['FSI_INDEX', 'TELCO_INDEX']:
    ...
    for i in range (0,future_days+1):
        df[sym, 'RD{:02d}'.format(i)] = ((temp_df.CLOSEPRICE.shift(-1*i) - temp_df.OPENPRICE) / temp_df.OPENPRICE)*100

## Sort On Symbol Name (Level 0)
df = df.sort_index(axis=1, level=0).round(2)\
      .loc[ date_from : date_to, : ]

df.to_csv('data/eod_returns.csv')
